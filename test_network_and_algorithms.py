...
def visualize_astar_path(occ_map: np.array, path: list, start, finish):
    ...
    for point in path:
        points_map = cv2.circle(points_map, (point[1], point[0]), 5, (255, 0, 0), -1)

    points_map = cv2.circle(points_map, (start[1], start[0]), 5, (0, 255, 0), -1)
    points_map = cv2.circle(points_map, (finish[1], finish[0]), 5, (0, 0, 255), -1)

    plt.imshow(cv2.cvtColor(points_map, cv2.COLOR_BGR2RGB), origin='lower')
    plt.show()
...
def main():
    ...
    calculate_time = timer_finish - timer_start
    print("CALCULATE_TIME:", calculate_time)
    df.to_excel('results_vol_2.xlsx', index=True)
